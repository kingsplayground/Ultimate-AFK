...
namespace UltimateAFK.Resources.Component
{
    public class NewComponent : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                Log.Error($"Error on {nameof(NewComponent)}::{nameof(Start)}: Error on try get player");
                Destroy(this);
                return;
            }
                ...
                case RoleTypeId.Scp096:
                    {
                        ...
                        // Player is moving or its crying o a wall/door
        ...
        private Player GetReplacement()
        {
            ...
            {
                Log.Error($"Error on {nameof(NewComponent)}::{nameof(GetReplacement)}: {e.Message} || typeof {e.GetType()}");
                return null;
            }
        }
...
/// </summary>
        private IEnumerator<float> AfkStatusChecker()
        {
            ...
            while (true)
            {
                ...
                {
                    Log.Error($"Error on {nameof(NewComponent)}::{nameof(AfkStatusChecker)}: {e.Message} || typeof {e.GetType()}");
                }
